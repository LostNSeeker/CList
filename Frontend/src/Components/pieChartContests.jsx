...
export function PieChartContests() {
  ...
  const chartData = [
    { 
      ...
      value: totalStats.lcContests || 0,
      platform: 'leetcode'
    },
    { 
      name: PLATFORM_STYLES.codeforces.name,
      value: totalStats.cfContests || 0,
      platform: 'codeforces'
    },
    { 
      name: PLATFORM_STYLES.codechef.name,
      value: totalStats.ccContests || 0,
      platform: 'codechef'
    }
  ...
  const CustomTooltip = ({ active, payload }) => {
    if (active && payload?.[0]) {
      ...
      const percentage = ((data.value / totalStats.totalContests) * 100).toFixed(1);
      ...
      return (
        <div className="bg-white p-3 shadow-lg rounded-lg border">
          ...
          <p className="text-sm text-gray-600">
            {data.value.toLocaleString()} contests
          </p>
      ...
      return (
        <>
          ...
          <text x={cx} y={cy} textAnchor="middle" dominantBaseline="central">
            <tspan x={cx} y={cy - 15} dy="0" textAnchor="middle" fill="#374151" fontSize="24" fontWeight="bold">
              {totalStats.totalContests.toLocaleString()}
            </tspan>
            <tspan x={cx} y={cy + 15} dy="0" textAnchor="middle" fill="#6B7280" fontSize="14">
              Contests
            </tspan>
          </text>
        </>
